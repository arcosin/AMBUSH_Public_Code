...
def main(args):
    ...
    dss = [
        BurnDS_TDI(os.path.join(args.data_dir, "pig1/d0"), aug_transform_b = aug, t = 0),
        BurnDS_TDI(os.path.join(args.data_dir, "pig1/d3"), aug_transform_b = aug, t = 3),
        BurnDS_TDI(os.path.join(args.data_dir, "pig1/d7"), aug_transform_b = aug, t = 7),
        ...
        BurnDS_TDI(os.path.join(args.data_dir, "pig2/d0"), aug_transform_b = aug, t = 0),
        BurnDS_TDI(os.path.join(args.data_dir, "pig2/d3"), aug_transform_b = aug, t = 3),
        BurnDS_TDI(os.path.join(args.data_dir, "pig2/d7"), aug_transform_b = aug, t = 7),
        ...
        BurnDS_TDI(os.path.join(args.data_dir, "pig3/d0"), aug_transform_b = aug, t = 0),
        BurnDS_TDI(os.path.join(args.data_dir, "pig3/d3"), aug_transform_b = aug, t = 3),
        BurnDS_TDI(os.path.join(args.data_dir, "pig3/d7"), aug_transform_b = aug, t = 7),
    ]
    valid_dss = [
        BurnDS_TDI(os.path.join(args.data_dir, "pig4/d0"), aug_transform_b = aug, t = 0),
        BurnDS_TDI(os.path.join(args.data_dir, "pig4/d3"), aug_transform_b = aug, t = 3),
        BurnDS_TDI(os.path.join(args.data_dir, "pig4/d7"), aug_transform_b = aug, t = 7),
        BurnDS_TDI(os.path.join(args.data_dir, "pig5/d0"), aug_transform_b = aug, t = 0),
        BurnDS_TDI(os.path.join(args.data_dir, "pig5/d3"), aug_transform_b = aug, t = 3),
        BurnDS_TDI(os.path.join(args.data_dir, "pig5/d7"), aug_transform_b = aug, t = 7),
    ]
    ...
    if args.check_full_ds:
        return
    elif args.vid_validate:
        assert args.val_fold in [3,5,7,9]
        train_inds_0 = train_ds_0.get_video_indices()
        train_inds_3 = train_ds_3.get_video_indices()
        train_inds_7 = train_ds_7.get_video_indices()
        val_inds_0, train_inds_0 = get_inds_from_region(train_inds_0, args.val_fold)
        val_inds_3, train_inds_3 = get_inds_from_region(train_inds_3, args.val_fold)
        val_inds_7, train_inds_7 = get_inds_from_region(train_inds_7, args.val_fold)
        val_ds_0 = Subset(train_ds_0, val_inds_0)
        val_ds_3 = Subset(train_ds_3, val_inds_3)
        val_ds_7 = Subset(train_ds_7, val_inds_7)
        train_ds_0 = Subset(train_ds_0, train_inds_0)
        train_ds_3 = Subset(train_ds_3, train_inds_3)
        train_ds_7 = Subset(train_ds_7, train_inds_7)
        valid_ds = ConcatDataset([val_ds_0, val_ds_3, val_ds_7])
        ...
        print("Split on region %d." % (args.val_fold,))
        print("Train dataset size (split):  %d." % len(train_ds))
        print("Valid dataset size (split):  %d." % len(valid_ds))
        dataloader = {"train": train_dl, "valid": valid_dl}
    elif args.validate:
        with open(args.fold_file, 'r', encoding = "utf-8") as f:
            folds = json.load(f)
        valid_ds = Subset(train_ds, folds[args.val_fold])
        other_folds = [x for i, x in enumerate(folds) if i != args.val_fold]
        train_ds = Subset(train_ds, [x for fold in other_folds for x in fold])
        train_dl = DataLoader(train_ds, batch_size = batch_size, shuffle = True, drop_last = True)
        valid_dl = DataLoader(valid_ds, batch_size = batch_size, shuffle = False, drop_last = False)
        print("Split on fold %d from fold file %s." % (args.val_fold, args.fold_file))
        print("Train dataset size (split):  %d." % len(train_ds))
        print("Valid dataset size (split):  %d." % len(valid_ds))
        dataloader = {"train": train_dl, "valid": valid_dl}
    else:
    ...
    torch.save(model.state_dict(), os.path.join(args.save_dir, "resnet_text_std.pth"))
    make_trainplot(args.save_dir, args.epochs, train_l, val_l, train_a, val_a)
    print("Train loss:  %s.\n" % train_l)
    print("Train accu:  %s.\n" % train_a)
    print("Valid loss:  %s.\n" % val_l)
    print("Valid accu:  %s.\n" % val_a)
...
def main_crossval(args):
    ...
    for fold_id in range(5):
        ...
        print("Saved model at %s." % os.path.join(args.save_dir, "model_fold_%d.pth" % fold_id))
...
def main_crossval_tdi_only(args):
    dev = torch.device("cpu" if args.cpu or not torch.cuda.is_available() else "cuda:%d" % args.gpu_id)
    batch_size = 32
    lr = 0.0001
    lrfe = lr * 0.25
    ...
    for fold_id in range(5):
        ...
        cw = get_class_weights(train_ds)
        print("CW:   %s." % str(cw))
        model = BurnClassifierType3(4, freeze_fe = full_freeze).to(dev)
        model_pars = model.params_split()
        if full_freeze:
            opt = optim.Adam(model.parameters(), lr = lr, weight_decay = 1e-5)
        else:
            ...
            opt = optim.Adam(pars, lr = lr, weight_decay = 1e-5)
        model, train_l, train_a, val_l, val_a = train_model(model, dataloader, opt, args.epochs, dev, cw = cw)
        torch.save(model.state_dict(), os.path.join(args.save_dir, "model_tdi_fold_%d.pth" % fold_id))
        print("Saved model at %s." % os.path.join(args.save_dir, "model_tdi_fold_%d.pth" % fold_id))
        make_trainplot(args.save_dir, args.epochs, train_l, val_l, train_a, val_a, p = "./training_plot_tdi_fold_%d.png" % fold_id)
        print("Train loss:  %s.\n" % train_l)
        print("Train accu:  %s.\n" % train_a)
        print("Valid loss:  %s.\n" % val_l)
        print("Valid accu:  %s.\n" % val_a)
